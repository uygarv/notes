...
export function NotesWorkspace() {
  ...
  const filtered = useMemo(() => (notes.data ?? []).filter((note) => {
    ...
    const searchableValues = [note.title, ...note.tags.map((tag) => tag.name)];
    if (!note.isLocked) searchableValues.push(toPlainText(note.content));
