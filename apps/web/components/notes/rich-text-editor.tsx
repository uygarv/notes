...
import { Bold, Check, ChevronDown, Code2, Columns2, Heading1, Heading2, Heading3, Italic, Link2, List, ListChecks, ListOrdered, Merge, Quote, Redo2, Rows2, Split, Strikethrough, Table2, Trash2, Type, Underline as UnderlineIcon, Undo2 } from 'lucide-react';
...
type BlockStyle = 'title' | 'heading' | 'subheading' | 'body' | 'monostyled' | 'bullet' | 'dashed' | 'numbered' | 'quote';
...
export function RichTextEditor({ content, onChange, editable = true }: RichTextEditorProps) {
  ...
  const selectionBeforeLinkDialog = useRef<{ from: number; to: number } | null>(null);
  const editor = useEditor({
    immediatelyRender: false,
    editable,
    extensions: [
      StarterKit.configure({ heading: { levels: [1, 2, 3] }, bulletList: false, link: false, underline: false }),
      StyledBulletList,
      Underline,
    ...
    editorProps: {
      attributes: {
        class: 'notes-prose min-h-[28rem] flex-1 px-0 py-2 outline-none',
      },
    },
  ...
  const openLinkDialog = useCallback(() => {
    if (!editor) return;
    selectionBeforeLinkDialog.current = { from: editor.state.selection.from, to: editor.state.selection.to };
  ...
  const saveLink = useCallback(() => {
    ...
    if (selectionBeforeLinkDialog.current) chain.setTextSelection(selectionBeforeLinkDialog.current);
    if (!value) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="bg-background/95 sticky top-14 z-10 -mx-1 flex flex-nowrap items-center gap-0.5 overflow-x-auto border-b py-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden backdrop-blur supports-[backdrop-filter]:bg-background/80">
      ...
      <EditorButton label="Italic" active={editor.isActive('italic')} onClick={() => editor.chain().focus().toggleItalic().run()}><Italic /></EditorButton>
      <EditorButton label="Underline" active={editor.isActive('underline')} onClick={() => editor.chain().focus().toggleUnderline().run()}><UnderlineIcon /></EditorButton>
      <EditorButton label="Strike through" active={editor.isActive('strike')} onClick={() => editor.chain().focus().toggleStrike().run()}><Strikethrough /></EditorButton>
      <EditorButton label="Inline code" active={editor.isActive('code')} onClick={() => editor.chain().focus().toggleCode().run()}><Code2 /></EditorButton>
      <Separator orientation="vertical" className="mx-1 h-5" />
    ...
    <EditorContent editor={editor} className="min-h-0 min-w-0 max-w-full flex-1 overflow-x-hidden" />
    <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{editor.isActive('link') ? 'Edit link' : 'Add link'}</DialogTitle>
          <DialogDescription>Paste a URL for the selected text. Leave it blank to remove the link.</DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); saveLink(); }}>
          <div className="space-y-2">
            <label htmlFor="note-link-url" className="text-sm font-medium">URL</label>
            <Input id="note-link-url" autoFocus value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder="https://example.com" inputMode="url" />
          </div>
...
function BlockStyleMenu({ editor }: { editor: Editor }) {
  ...
  function applyStyle(style: BlockStyle) {
    ...
    if (style === 'bullet' || style === 'dashed') {
      ...
      editor.chain().focus().updateAttributes('bulletList', { listStyle: style === 'dashed' ? 'dash' : 'bullet' }).run();
    }
  }
...
function TableControls({ editor }: { editor: Editor }) {
  ...
  if (!tableState.active) {
    return <EditorButton label="Insert 3 by 3 table" onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()}><Table2 /></EditorButton>;
  }

  const run = (command: () => boolean) => command();

  return <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="secondary" size="sm" className="h-7 gap-1 px-2" aria-label="Edit table"><Table2 className="size-3.5" /><ChevronDown className="size-3" /></Button></DropdownMenuTrigger><DropdownMenuContent align="start" className="min-w-52"><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().addRowBefore().run())}><Rows2 />Add row above</DropdownMenuItem><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().addRowAfter().run())}><Rows2 />Add row below</DropdownMenuItem><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().deleteRow().run())}><Trash2 />Delete row</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().addColumnBefore().run())}><Columns2 />Add column left</DropdownMenuItem><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().addColumnAfter().run())}><Columns2 />Add column right</DropdownMenuItem><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().deleteColumn().run())}><Trash2 />Delete column</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => run(() => editor.chain().focus().toggleHeaderRow().run())}><Heading1 />Toggle header row</DropdownMenuItem><DropdownMenuItem disabled={!tableState.canMerge} onSelect={() => run(() => editor.chain().focus().mergeCells().run())}><Merge />Merge cells</DropdownMenuItem><DropdownMenuItem disabled={!tableState.canSplit} onSelect={() => run(() => editor.chain().focus().splitCell().run())}><Split />Split cell</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={() => run(() => editor.chain().focus().deleteTable().run())}><Trash2 />Delete table</DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}

function BlockStyleItem({ active, icon, label, onSelect }: { active: boolean; icon: React.ReactNode; label: string; onSelect: () => void }) {
  return <DropdownMenuItem onSelect={onSelect}>{active ? <Check /> : icon}<span>{label}</span></DropdownMenuItem>;
}

function getCurrentBlockStyle(editor: Editor) {
  if (editor.isActive('heading', { level: 1 })) return { id: 'title', label: 'Title' };
  if (editor.isActive('heading', { level: 2 })) return { id: 'heading', label: 'Heading' };
  if (editor.isActive('heading', { level: 3 })) return { id: 'subheading', label: 'Subheading' };
  if (editor.isActive('codeBlock')) return { id: 'monostyled', label: 'Monostyled' };
  if (editor.isActive('bulletList', { listStyle: 'dash' })) return { id: 'dashed', label: 'Dashed list' };
  if (editor.isActive('bulletList')) return { id: 'bullet', label: 'Bulleted list' };
  if (editor.isActive('orderedList')) return { id: 'numbered', label: 'Numbered list' };
  if (editor.isActive('blockquote')) return { id: 'quote', label: 'Block quote' };
