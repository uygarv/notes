...
function UserMenu({ user, onLogout, onOpenSettings }: { user: { email: string; username: string | null; profileImageUrl: string | null } | undefined; onLogout: () => void; onOpenSettings?: () => void }) {
  const name = user?.username || user?.email || 'N';
  return <DropdownMenu><DropdownMenuTrigger asChild><button className="flex w-full items-center gap-2 rounded-md p-2 text-left text-sm outline-none transition hover:bg-sidebar-accent focus-visible:ring-2 focus-visible:ring-sidebar-ring"><UserAvatar name={name} imageUrl={user?.profileImageUrl} className="size-8 rounded-lg" fallbackClassName="rounded-lg" /><span className="grid min-w-0 flex-1 leading-tight"><span className="truncate font-medium">{user?.username || 'Your workspace'}</span><span className="truncate text-xs text-muted-foreground">{user?.email}</span></span><ChevronsUpDown className="size-4 text-muted-foreground" /></button></DropdownMenuTrigger><DropdownMenuContent className="w-60" side="top" align="end"><DropdownMenuLabel className="font-normal"><div className="flex items-center gap-2"><UserAvatar name={name} imageUrl={user?.profileImageUrl} className="size-8 rounded-lg" fallbackClassName="rounded-lg" /><div className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-medium">{user?.username || 'Your workspace'}</span><span className="truncate text-xs text-muted-foreground">{user?.email}</span></div></div></DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuGroup><DropdownMenuItem asChild onSelect={onOpenSettings}><Link href="/settings"><Settings />Settings</Link></DropdownMenuItem></DropdownMenuGroup><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={onLogout}><LogOut />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}

function DesktopSidebar({ pathname, user, onNewNote, onLogout }: { pathname: string; user: { email: string; username: string | null; profileImageUrl: string | null } | undefined; onNewNote: () => void; onLogout: () => void }) {
...
function MobileSidebar({ open, onOpenChange, pathname, user, onNewNote, onLogout }: { open: boolean; onOpenChange: (open: boolean) => void; pathname: string; user: { email: string; username: string | null; profileImageUrl: string | null } | undefined; onNewNote: () => void; onLogout: () => void }) {
