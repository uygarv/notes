...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isSignup) await unwrap(await api.auth.signUp({ body: validation.data }));
  ...
  return <main className="relative flex min-h-svh flex-col items-center justify-center gap-6 bg-muted/50 p-6 md:p-10">
    ...
    <motion.div initial={{ opacity: 0, y: 16, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.38, ease: [0.22, 1, 0.36, 1] }} className="flex w-full max-w-sm flex-col gap-6">
      <Link href="/" className="flex items-center gap-2 self-center font-medium"><span className="flex size-6 items-center justify-center rounded-md bg-primary text-primary-foreground"><GalleryVerticalEnd className="size-4" /></span>Notes</Link>
      <Card>
        <CardHeader className="text-center"><CardTitle className="text-xl">{isSignup ? 'Create your account' : 'Welcome back'}</CardTitle><CardDescription>{isSignup ? 'Start a private, focused home for your notes.' : 'Sign in to continue to your workspace.'}</CardDescription></CardHeader>
        <CardContent><form onSubmit={submit} noValidate><FieldGroup>
          <Field className="grid grid-cols-2 gap-3"><Button variant="outline" type="button" onClick={() => window.location.assign(apiUrl('/auth/google'))} disabled={pending}><GoogleIcon className="size-4" /> {oauthProviders.google.name}</Button><Button variant="outline" type="button" onClick={() => window.location.assign(apiUrl('/auth/github'))} disabled={pending}><GitHubIcon className="size-4" /> {oauthProviders.github.name}</Button></Field>
      ...
      <FieldDescription className="px-6 text-center text-xs">By continuing, you agree to use Notes responsibly.</FieldDescription>
    </motion.div>
  </main>;
}
